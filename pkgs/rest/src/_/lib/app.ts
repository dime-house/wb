...
import cloudRouter from './controllers/cloud.js';
...
import instancesRouter from './controllers/instances.js';
...
import namespacesRouter from './controllers/cloud/namespaces.js';
import {
	isInstalled,
...
import { getCloudEvents } from './cloud/events.js';
import { fileURLToPath } from 'url';
import context from '@/middleware/context.js';
import { controllers } from '@/@dime/controlers/index.js';
import type { ControllerInstance } from '@/@dime/types.js';
...
export default async function createApp(): Promise<express.Application> {
	...
	Context.ca = await readFile(`${rootDir}/certs/ca.pem`);
	Context.cert = await readFile(`${rootDir}/certs/client-cert.pem`);
	Context.key = await readFile(`${rootDir}/certs/client-key.pem`);
	...
	const cloudEvents = getCloudEvents();

	await extensionManager.initialize();
	await flowManager.initialize();
	await cloudEvents.initialize();
	...
	controllers.forEach((controller: ControllerInstance) => {
		app.use(controller.$options.path, controller.$router);
	});
