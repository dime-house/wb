import type { FieldFilter } from '@dime/type';
import type {
  AnySchema,
  DateSchema,
  NumberSchema,
...
export const Joi: typeof BaseJoi = BaseJoi.extend({
  ...
  messages: {
    ...
    'string.icontains':
      '{{#label}} must contain case insensitive [{{#substring}}]',
  ...
  if (Object.keys(value)[0]?.startsWith('_') === false) {
    ...
    if (operator === '_contains') {
      if (
        ...
        schema[key] = Joi.alternatives().try(
          ...
          Joi.array().items(
            getStringSchema().contains(compareValue).required(),
            Joi.any()
          )
        );
      }
    ...
    if (operator === '_icontains') {
      if (
        ...
        schema[key] = Joi.alternatives().try(
          ...
          Joi.array().items(
            getStringSchema().icontains(compareValue).required(),
            Joi.any()
          )
        );
      }
    ...
    if (operator === '_ncontains') {
      if (
        ...
        schema[key] = Joi.alternatives().try(
          ...
          Joi.array().items(
            getStringSchema().contains(compareValue).forbidden()
          )
        );
      }
    }
    ...
    if (operator === '_istarts_with') {
      if (
        ...
        schema[key] = getStringSchema().pattern(
          new RegExp(`^${escapeRegExp(compareValue)}.*`, 'i'),
          {
            name: 'istarts_with',
          }
        );
