...
export function parseFilterFunctionPath(path: string): string {
  if (path.includes('(') && path.includes(')')) {
    ...
    const preColumns = preHasColumns
      ? pre.slice(0, pre.lastIndexOf('.') + 1)
      : '';
    const functionName = preHasColumns
      ? pre.slice(pre.lastIndexOf('.') + 1)
      : pre;
    ...
    if (matched) {
      ...
      const columns = fieldsHasColumns
        ? fields.slice(0, fields.lastIndexOf('.') + 1)
        : '';
      const field = fieldsHasColumns
        ? fields.slice(fields.lastIndexOf('.') + 1)
        : fields;
