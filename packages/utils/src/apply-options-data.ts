import type { JsonValue } from '@dime/type';
...
export function applyOptionsData(
  ...
  return Object.fromEntries(
    Object.entries(options).map(([key, value]) => [
      key,
      renderMustache(value, data, skipUndefinedKeys.includes(key)),
    ])
  );
}
...
function renderMustache<T extends JsonValue>(
  ...
  if (typeof item === 'string') {
    ...
    return renderFn(item, resolveFn(skipUndefined) as ResolveFn, scope, {
      explicit: true,
    }) as Mustache<T>;
  } else if (Array.isArray(item)) {
    return item.map((element) =>
      renderMustache(element, scope, skipUndefined)
    ) as Mustache<T>;
  } else if (typeof item === 'object' && item !== null) {
    return Object.fromEntries(
      Object.entries(item).map(([key, value]) => [
        key,
        renderMustache(value, scope, skipUndefined),
      ])
    ) as Mustache<T>;
  } else {
