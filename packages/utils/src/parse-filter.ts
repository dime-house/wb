import { REGEX_BETWEEN_PARENS } from '@dime/const';
import type { Accountability, Filter, Policy, Role, User } from '@dime/type';
...
function parseFilterRecursive(
  ...
  const filters = Object.entries(filter).map((entry) =>
    parseFilterEntry(entry, accountability, context)
  );
...
export function parsePreset(
  ...
  return deepMap(preset, (value) =>
    parseFilterValue(value, accountability, context)
  );
}
...
function parseFilterEntry(
  ...
  if (['_or', '_and'].includes(String(key))) {
    return {
      [key]: value.map((filter: Filter) =>
        parseFilterRecursive(filter, accountability, context)
      ),
    };
    ...
    return {
      [`item:${String(key).split('item__')[1]}`]: parseFilter(
        value,
        accountability,
        context
      ),
    ...
    return {
      [key]: parseFilterRecursive(value, accountability, context),
    } as Filter;
  }
}
...
function parseDynamicVariable(
  ...
  if (value.startsWith('$CURRENT_POLICIES')) {
    if (value === '$CURRENT_POLICIES')
      return (
        (get(context, value, null) as Policy[] | null)?.map(({ id }) => id) ??
        null
      );
    return get(context, value, null);
  }
