...
import { toArray } from '@dime/utils';
...
export function parseEnv(prefix: string, options?: parseEnvOptions) {
  ...
  const omitKeys = toArray(options?.omitKey ?? []).map((key) =>
    key.toLowerCase()
  );
  const omitPrefixes = toArray(options?.omitPrefix ?? []).map((prefix) =>
    prefix.toLowerCase()
  );
  ...
  for (const [key, value] of Object.entries(env)) {
    ...
    if (omitKeys.length > 0) {
      const isKeyInOmitKeys = omitKeys.some(
        (keyToOmit) => lowerCaseKey === keyToOmit
      );
      if (isKeyInOmitKeys) continue;
    }

    if (omitPrefixes.length > 0) {
      const keyStartsWithAnyPrefix = omitPrefixes.some((prefix) =>
        lowerCaseKey.startsWith(prefix)
      );
